    ...
    case "assign": {
      ...
      if (symbol.kind === "global") {
        if (symbol.type.kind === "aggregate") {
          return analyzeAggregateAssignStmt(symbol as BoundGlobalSymbol & { type: SemanticAggregateType }, stmt.expr, scope, functionSymbols, functionName, sourceText, file, stmt.isInitialization);
        }
        return {
          kind: "expr",
          expr: {
      ...
      if (symbol.type.kind === "aggregate") {
        return analyzeAggregateAssignStmt(symbol as BoundLocalSymbol & { type: SemanticAggregateType }, stmt.expr, scope, functionSymbols, functionName, sourceText, file, stmt.isInitialization);
      }
      return {
        kind: "assign",
        local: symbol,
    ...
    ? (() => {
      ...
      return {
        pointer: {
          ...
          index: { kind: "const" as const, value: field.offset, type: toSemanticScalarType("int") },
          pointee: "char" as const,
          type: toSemanticPointerType("char"),
        },
        type: field.type,
      };
...
function getArrayFieldAddressFromConsumerExpr(
  ...
  const arrayPointerType = (type: SemanticArrayType): SemanticPointerType =>
    toSemanticPointerType(getArrayDecayPointee(type));
  const makeAddress = (pointer: BoundExpr, field: AggregateFieldLayout): BoundExpr => ({
    kind: "pointerAdd",
    pointer,
    index: { kind: "const", value: field.offset, type: toSemanticScalarType("int") },
    pointee: "char",
    type: arrayPointerType(field.type as SemanticArrayType),
  });
  ...
  return {
    ...
    type: arrayPointerType(toSemanticType(field.type) as SemanticArrayType),
  };
}
  ...
  switch (expr.kind) {
    ...
    case "memberAccess": {
      ...
      if (rawField.type.kind === "array") {
        return getArrayFieldAddressFromConsumerExpr(expr, scope, functionSymbols, functionName, sourceText, file) as BoundExpr;
      }
    ...
    case "arrayIndex": {
      ...
      if (symbol.type.elementValueType || symbol.type.dimensions?.length) {
        ...
        const elementAddress: BoundExpr = {
          kind: "pointerAdd",
          pointer: symbol.kind === "global"
...
function toSemanticScalarType(type: ScalarType, qualifiers?: TypeQualifiers): SemanticScalarType {
  return {
    kind: "scalar",
    name: type,
    width: type === "char" ? 1 : 2,
...
function toSemanticPointerType(pointee: PointerPointee, qualifiers?: TypeQualifiers, pointeeQualifiers?: TypeQualifiers): SemanticPointerType {
  return {
    kind: "pointer",
    pointee,
    width: 2,
...
function hasTypeQualifiers(qualifiers: TypeQualifiers | undefined): boolean {
  return Boolean(qualifiers?.isConst || qualifiers?.isVolatile || qualifiers?.isRestrict);
}

function assertModifiableType(
  ...
  if (type.qualifiers?.isConst) {
    throwDiagnostic(sourceText, `TsSccCompilerAdapter C Subset cannot modify const-qualified ${description} in ${functionName}().`, { file, offset: 0 });
  }
}

function assertModifiablePointee(
...
function assertModifiableArrayElement(
  type: SemanticArrayType,
  description: string,
  functionName: string,
  sourceText: string,
  file?: string,
...
function getArrayDecayPointee(type: SemanticArrayType): PointerPointee {
  ...
  const build = (dimensions: number[]): PointerPointee => {
    if (dimensions.length === 0) return element;
...
function getArrayIndexPointee(type: SemanticArrayType): PointerPointee {
  const remaining = type.dimensions ?? [];
  if (remaining.length === 0) return toArrayElementPointee(type);
  const element = toArrayElementPointee(type);
  const build = (dimensions: number[]): PointerPointee => {
    if (dimensions.length === 0) return element;
    const [length, ...tail] = dimensions;
...
function getReadableAggregateFieldLayout(
  ...
  throwDiagnostic(sourceText, `TsSccCompilerAdapter Phase C subset only supports scalar/pointer/function-pointer field assignment or access on ${type.aggregateKind} ${type.name}.${fieldName} in ${functionName}().`, {
    file,
    offset: 0,
  });
}
