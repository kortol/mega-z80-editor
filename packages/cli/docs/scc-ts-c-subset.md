...
| X23 | `a[row][column]`, `p[row][column]`, `(*p)[i]` | 2-D `char[M][N]` / `int[M][N]`, sized `char (*)[N]` / `int (*)[N]` | S | S | S | S | S | local/global/parameter storage, initializer, field, return/call, conditional/comma/assignment value transport are runtime-covered |
