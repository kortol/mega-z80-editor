...
この時点で source path は `while` だけでなく `for` でも loop/back-edge を作れます。test では additive / unary minus / logical not / logical and-or / bitwise / bitwise-not / multiplicative / shift expression を inline arithmetic / helper arithmetic / helper compare / short-circuit branch へ lower し、`for` + local declaration init + `continue` + `break` を含む source を CP/M 実行まで確認しています。
...
未対応:
- full C statement coverage (`switch`, richer unary/binary operators, pointers, arrays)
