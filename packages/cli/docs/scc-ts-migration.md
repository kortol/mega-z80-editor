...
| assign expression result | S | S | N | N |
| address-of | S | S | S | N |
...
- `tsFrontendSemantic.ts`
  - aggregate value path は `call` / `conditional` / `comma` / field-read まで入った
  - ただし aggregate value 自体を scalar expression と同列に扱う汎化はまだない
  - compare / truthiness / assign-expression result などは未対応
- `tsProgram.ts`
...
- `tsFrontendLowering.ts`
  ...
  - branch / conditional / comma をまたぐ aggregate temporary path は増えたが、assign-expression result などは未対応
...
イギリスや日本のような、王や天皇がおられて、その下に選ばれた人が君臨するシステムとの
...
英国では、英連邦諸国の政治的合意を経て、英国議会が2013年王位継承法を成立させ、2011年10月28日以降に生まれた王族について、弟が姉を追い越す男子優先制度を廃止しました。つまり、女王の治世下で実現したが、決定主体は議会と関係諸国でした

日本が論じるべきなのは、
主権在民の下で、政治権力を持たない天皇が、国家の継続性と国民統合を象徴する制度を、21世紀にふさわしい形でどう維持するかである。

そのためには、皇位継承を男女にかかわらない長子優先制へ改めることが、最も自然で安定した道ではないか。
