...
import { RelModule, LinkResult, RelSectionInfo, LinkedSymbol } from "./types";
...
function inferSectionKind(name?: string, kind?: RelSectionInfo["kind"]): RelSectionInfo["kind"] {
  if (kind) return kind;
...
function buildModuleSections(mod: RelModule): RelSectionInfo[] {
  if (mod.sections && mod.sections.length > 0) return mod.sections;
  const order: string[] = [];
  for (const t of mod.texts) {
    const name = t.section ?? ".text";
    const key = name.toLowerCase();
    if (!order.some((n) => n.toLowerCase() === key)) order.push(name);
  }
  if (order.length === 0) order.push(".text");
  return order.map((name, idx) => ({ id: idx, name }));
}

function sectionSize(mod: RelModule, secName: string, info?: RelSectionInfo): number {
  if (info?.size && info.size > 0) return info.size;
  let max = 0;
  for (const t of mod.texts) {
    if (!t.section) continue;
    if (normalizeSectionName(t.section) !== normalizeSectionName(secName)) continue;
...
export function linkModulesV2(
  mods: RelModule[],
  opts: { orgText?: number; orgData?: number; orgBss?: number; orgCustom?: number } = {}
): LinkResult {
  const symbols = new Map<string, LinkedSymbol>();
  const warnings: string[] = [];
  const texts: { addr: number; bytes: number[] }[] = [];
  ...
  const sectionBase = new Map<string, number>();
  const sectionKey = (modIndex: number, sectionName: string) => `${modIndex}::${normalizeSectionName(sectionName)}`;
  const moduleDefaultSection = new Map<number, string>();
  ...
  const textSections: { modIndex: number; sec: RelSectionInfo; size: number }[] = [];
  const dataSections: { modIndex: number; sec: RelSectionInfo; size: number }[] = [];
  const bssSections: { modIndex: number; sec: RelSectionInfo; size: number }[] = [];
  const customSections: { modIndex: number; sec: RelSectionInfo; size: number }[] = [];
  const asegSections: { modIndex: number; sec: RelSectionInfo; size: number }[] = [];
  ...
  moduleSections.forEach((sections, modIndex) => {
    ...
    for (const sec of sections) {
      ...
      if (kind === "TEXT") textSections.push(entry);
      else if (kind === "DATA") dataSections.push(entry);
      else if (kind === "BSS") bssSections.push(entry);
      else if (kind === "ASEG") asegSections.push(entry);
      else customSections.push(entry);
    }
  });
  ...
  const sectionMeta = new Map<string, { kind: RelSectionInfo["kind"]; base: number; size: number }>();
  ...
  const segmentOrder: Array<NonNullable<RelSectionInfo["kind"]>> = ["TEXT", "DATA", "BSS", "CUSTOM"];
  const detailByKind = new Map<NonNullable<RelSectionInfo["kind"]>, { name: string; base: number; size: number; align?: number; org?: number }[]>();
  ...
  const segments: LinkResult["segments"] = [];
  ...
  for (const kind of segmentOrder) {
    ...
    const kindLower = kind.toLowerCase() as "text" | "data" | "bss" | "custom";
    segments.push({
      bank: 0,
      kind: kindLower,
      range: { min, max },
  ...
  const segmentDetails = segments.map((seg) => {
    const kindUpper = seg.kind.toUpperCase() as NonNullable<RelSectionInfo["kind"]>;
    return {
      kind: seg.kind,
...
function formatDefinedAt(defFile?: string, defLine?: number): string | undefined {
  if (!defFile) {
