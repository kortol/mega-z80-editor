...
export function parse(tokens: Token[]): Node[] {
  ...
  function flushLine() {
    ...
    // ラベル（ident + colon）
    if (line.length >= 2 && line[0].kind === "ident" && line[1].kind === "colon") {
        nodes.push({ kind: "label", name: line[0].text, line: line[0].line });
        line = line.slice(2);
        if (line.length === 0) return;
    }
    ...
    if (line.length >= 3 && line[1].kind === "ident" && line[1].text.toUpperCase() === "EQU") {
        const symbol = line[0].text;
        const valueTokens = line.slice(2).filter(t => t.kind !== "comma").map(t => t.text);
        nodes.push({ kind: "pseudo", op: "EQU", args: [symbol, ...valueTokens], line: line[0].line });
        return;
    }

    // 通常の命令 or 疑似命令
    const op = line[0].text.toUpperCase();
    const args = line.slice(1).filter(t => t.kind !== "comma").map(t => t.text);
