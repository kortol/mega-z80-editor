...
import { parseIndexAddr, regCode } from "./utils";
...
export function encodeCB(ctx: AsmContext, node: NodeInstr) {
  ...
  if (op in rotMap) {
    ...
    if (idx) {
      emitBytes(ctx, [idx.prefix, 0xcb, idx.disp, rotMap[op] | 0x06], node.pos);
      return;
    }
    const reg = regCode(r);
  ...
  if (["BIT", "RES", "SET"].includes(op)) {
    ...
    if (idx) {
      ...
      emitBytes(ctx, [idx.prefix, 0xcb, idx.disp, base | (bit << 3) | 0x06], node.pos);
      return;
    }
    const reg = regCode(r);
  ...
  throw new Error(
    `Unsupported CB instruction ${op} ${args.join(",")} (supported: ${supported.join("; ")})`
  );
}
