...
function lowerGlobalStorage(global: BoundGlobalSymbol, globalsByName: ReadonlyMap<string, BoundGlobalSymbol>): { data?: DataSpec[]; bss?: DataSpec[] } {
...
function lowerGlobalArrayInitializer(
  initializer: BoundGlobalSymbol["initializer"],
  length: number,
  type: Extract<SourceType | SemanticType, { kind: "array" }>,
  globalsByName?: ReadonlyMap<string, BoundGlobalSymbol>,
): string {
  ...
  if (type.elementType === "int" && initializer?.kind === "list") {
    const values = flattenGlobalArrayInitializerItems(initializer, [type.length ?? 0, ...(type.dimensions ?? [])], "<array>").map((item) =>
      item?.kind === "expr" && item.expr.kind === "const" ? `${item.expr.value}` : "0");
    return values.join(",");
  }
  return lowerGlobalArrayInitializerBytes(
    initializer,
    length,
    [type.length ?? 0, ...(type.dimensions ?? [])],
  ).join(",");
}

function flattenGlobalArrayInitializerItems(
...
function getArrayElementCount(type: Extract<SourceType | SemanticType, { kind: "array" }>): number {
  return [type.length ?? 0, ...(type.dimensions ?? [])].reduce((count, length) => count * length, 1);
}

function getArrayStorageBytes(type: Extract<SourceType | SemanticType, { kind: "array" }>): number {
  const elementBytes = !type.elementValueType
    ? (type.elementType === "char" ? 1 : 2)
    : type.elementValueType.kind === "aggregate"
      ? ("size" in type.elementValueType ? type.elementValueType.size : getAggregateLayoutSize(type.elementValueType))
      : 2;
  return getArrayElementCount(type) * elementBytes;
}

function lowerGlobalArrayInitializerBytes(
  initializer: BoundGlobalSymbol["initializer"],
  length: number,
  dimensions = [length],
): string[] {
  ...
  if (initializer.kind === "list") {
    return flattenGlobalArrayInitializerItems(initializer, dimensions, "<array>").map((item) =>
      item?.kind === "expr" && item.expr.kind === "const" ? `${item.expr.value}` : "0");
  }
  return Array.from({ length }, () => "0");
}
...
function flattenGlobalAggregateArrayInitializer(
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    values.push(...flattenGlobalAggregateInitializer(
      index === 0 ? outputLabel : undefined,
      `${outputLabel ?? "<array>"}[${index}]`,
      elementType,
      item,
      globalsByName,
      false,
...
function normalizeFlatGlobalAggregateArrayItems(
  initializer: BoundGlobalSymbol["initializer"],
  dimensions: number[],
  elementType: Extract<SourceType | SemanticType, { kind: "aggregate" }>,
  label: string,
...
function getFlatGlobalAggregateInitializerWidth(
  ...
  let width = 0;
  for (const field of getAggregateLayoutFields(type)) {
...
function getFlatGlobalInitializerWidth(type: SourceType): number | undefined {
...
function consumeFlatGlobalAggregateInitializer(
  type: Extract<SourceType | SemanticType, { kind: "aggregate" }>,
...
function consumeFlatGlobalInitializerValue(
  type: SourceType,
  items: SourceInitializer[],
  start: number,
...
function flattenGlobalAggregateArrayItems(
  initializer: BoundGlobalSymbol["initializer"],
  dimensions: number[],
  label: string,
): Array<SourceInitializer | undefined> {
...
function flattenGlobalAggregateInitializer(
  ...
  initializer?: BoundGlobalSymbol["initializer"],
  globalsByName?: ReadonlyMap<string, BoundGlobalSymbol>,
  emitFallbackLabel = true,
): DataSpec[] {
  if (!initializer) {
    return [{
      label: outputLabel ?? (emitFallbackLabel ? name : undefined),
      directive: ".db",
      value: Array.from({ length: "size" in type ? type.size : getAggregateLayoutSize(type) }, () => "0").join(","),
    }];
  }
  if (initializer.kind !== "list") {
...
function lowerGlobalWordInitializer(initializer?: SourceInitializer, globalsByName?: ReadonlyMap<string, BoundGlobalSymbol>): string {
  if (!initializer) {
    return "0";
  }
...
function lowerGlobalFunctionPointerInitializer(initializer?: SourceInitializer, globalsByName?: ReadonlyMap<string, BoundGlobalSymbol>): string {
...
function resolveStaticGlobalFieldAddress(
  expr: SourceExpr,
  globalsByName: ReadonlyMap<string, BoundGlobalSymbol> | undefined,
): { name: string; offset: number; type: SourceType } | null {
  if (!globalsByName) {
    return null;
  }
...
function getAggregateFieldOffset(
  type: Extract<SourceType | SemanticType, { kind: "aggregate" }>,
  name: string,
): { offset: number; type: SourceType } | null {
  let offset = 0;
  for (const field of getAggregateLayoutFields(type)) {
    if (field.name === name) {
...
function lowerForInit(
  ...
  const aggregateInitializer = init.initializer as BoundAggregateValueExpr | BoundExpr;
  if (aggregateInitializer.type.kind === "aggregate") {
    return [
      ...materializeAggregateProducer(
      aggregateInitializer as BoundAggregateValueExpr,
      { kind: "localSlot", slot: init.local.slot, type: aggregateInitializer.type },
      externs,
      definedFunctions,
      sourceText,
      state,
  ...
  return [{
    ...
    slot: init.local.slot,
    width: getLocalValueWidth(init.local),
    expr: lowerExpr(init.initializer as BoundExpr, externs, definedFunctions, sourceText, state, functionState, file),
  }, ...initStatements];
}
...
function lowerAggregateAssignWrapper(
  target: BoundLocalSymbol | BoundGlobalSymbol | { kind: "aggregateAddress"; pointer: BoundExpr; type: SemanticAggregateType },
...
type AggregateDestination =
  ...
  | { kind: "pointer"; pointer: ExprIR; type: SemanticAggregateType }
...
function materializeAggregateProducer(
  ...
  switch (source.kind) {
    ...
    case "aggregateAddress":
      return lowerAggregateSourceAddressToDestination(
        lowerExpr(source.pointer, externs, definedFunctions, sourceText, state, functionState, file),
        source.type,
        destination,
      );
...
function lowerAggregateDestination(destination: AggregateDestination): AggregateDestinationIR {
...
function copyAggregateLocalSlotToDestination(
  ...
  switch (destination.kind) {
    ...
    case "pointer":
      return lowerAggregateCopySourceAddressToPointer(
        { kind: "localAddress", slot: sourceSlot },
        destination.pointer,
        destination.type,
      );
    case "returnSlot":
...
function lowerAggregateCopySourceAddressToPointer(
  sourcePointer: ExprIR,
  targetPointer: ExprIR,
  targetType: SemanticAggregateType,
): StmtIRHigh[] {
  return getAggregateFieldStores(targetType).map((field) => ({
    kind: "evalExpr",
...
function lowerAggregateCallIntoLocalSlot(
  targetSlot: number,
  source: Extract<BoundAggregateValueExpr, { kind: "call" | "indirectCall" }>,
  externs: Set<string>,
  definedFunctions: Set<string>,
  sourceText: string,
  state: LoweringState,
...
function getArrayPointerElementBytes(type: Extract<import("./tsFrontendAst").PointerPointee, { kind: "arrayPointer" }>): number {
...
function getPointerPointeeBytes(type: import("./tsFrontendAst").PointerPointee): number {
  ...
  if (type.kind === "aggregate") {
    return getAggregateLayoutSize(type);
  }
  return 2;
}

function lowerAggregateProducerExpr(
...
function aggregateProducerNeedsFieldConsumerTemp(source: AggregateProducerIR): boolean {
  switch (source.kind) {
    case "aggregateRef":
    case "aggregateAddress":
    case "aggregateAssignExpr":
