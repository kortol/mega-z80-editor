...
describe("encoder", () => {
    test("unsupported LD form → error", () => {
        const ctx = makeCtx();
        expect(() => (0, encoder_1.encodeInstr)(ctx, makeNode("LD", ["HL", "A"]))).toThrow(/Unsupported LD/);
    });
    test("unsupported instruction → error", () => {
        const ctx = makeCtx();
        expect(() => (0, encoder_1.encodeInstr)(ctx, makeNode("FOOBART", []))).toThrow(/Unsupported instruction FOOBAR/);
    });
});
