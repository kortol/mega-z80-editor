...
function evalIfDef(ctx, node, negate = false) {
    const symRaw = node.args?.[0]?.value?.trim() ?? "";
    if (!symRaw) {
        ctx.errors.push((0, errors_1.makeError)(errors_1.AssemblerErrorCode.SyntaxError, `${negate ? "IFNDEF" : "IFDEF"} requires symbol`, { pos: node.pos }));
        return false;
    }
...
function evalIfBlank(ctx, node, negate = false) {
    const raw = node.args?.[0]?.value;
    if (raw == null) {
        ctx.errors.push((0, errors_1.makeError)(errors_1.AssemblerErrorCode.SyntaxError, `${negate ? "IFNB" : "IFB"} requires <text>`, { pos: node.pos }));
        return false;
    }
...
function handleConditional(ctx, node) {
    ...
    if (op === "IF" || op === "IFIDN" || op === "IFDIF" || op === "IFDEF" || op === "IFNDEF" || op === "IFB" || op === "IFNB") {
        ...
        if (parentActive) {
            if (op === "IF")
                cond = evalConditionExpr(ctx, node.args?.[0]?.value ?? "", node.pos);
            else if (op === "IFIDN")
                cond = evalIfIdn(ctx, node);
            else if (op === "IFDIF")
                cond = !evalIfIdn(ctx, node);
            else if (op === "IFDEF")
                cond = evalIfDef(ctx, node, false);
            else if (op === "IFNDEF")
                cond = evalIfDef(ctx, node, true);
            else if (op === "IFB")
                cond = evalIfBlank(ctx, node, false);
            else if (op === "IFNB")
                cond = evalIfBlank(ctx, node, true);
        }
        stack.push({
            parentActive,
