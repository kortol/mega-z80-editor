...
const errors_1 = require("../../assembler/errors");
...
function formatPegExpected(expected) {
    if (expected.description)
        return expected.description;
    if (expected.type === "literal")
        return expected.text ? `'${expected.text}'` : "literal";
    if (expected.type === "class") {
        if (!expected.parts?.length)
            return "character";
        const parts = expected.parts.slice(0, 3).map((part) => Array.isArray(part) ? `${part[0]}-${part[1]}` : part);
...
function describePegSyntaxError(err) {
    ...
    const options = Array.from(new Set(expected.map((item) => formatPegExpected(item))));
...
function rawArgsFromDirectiveValues(values) {
...
function parsePeg(ctx, source) {
    ...
    try {
        ...
        if (err?.name === "SyntaxError" || /Expected/.test(String(err?.message ?? ""))) {
            throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.SyntaxError, describePegSyntaxError(err), { pos: locToPos(ctx, err?.location) });
        }
        throw err;
    }
    const nodes = [];
        ...
        if (item.type === "line") {
            ...
            if (labelName?.startsWith(".") && !labelColon) {
                ...
                if (instr.type === "macroInvoke") {
                    const args = [String(instr.name), ...(instr.args ?? []).map((a) => (a == null ? "" : String(a)))];
                    nodes.push(makePseudo(dottedOp, args.filter((value) => value.length > 0).map((value) => ({ value })), linePos));
                    continue;
                }
            }
            ...
            if (instr.type === "macroInvoke") {
                ...
                if (isOpcode && !hasMacro(instr.name)) {
                    const normalizedArgs = invokeArgs.map((arg) => normalizeSjasmOperandAlias(ctx, op, arg));
