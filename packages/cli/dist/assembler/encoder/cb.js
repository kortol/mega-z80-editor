...
exports.cbBitInstr = exports.cbRotInstr = void 0;
...
exports.cbRotInstr = [
    {
        match: (_ctx, args) => args.length === 1 &&
            (args[0].kind === operandKind_1.OperandKind.REG8 || args[0].kind === operandKind_1.OperandKind.REG_IND || args[0].kind === operandKind_1.OperandKind.IDX),
        encode(ctx, [arg], node) {
            const op = node.op.toUpperCase();
            const base = rotMap[op];
            if (base == null)
                throw new Error(`Unsupported CB instruction ${op}`);
            if (arg.kind === operandKind_1.OperandKind.IDX) {
                const prefix = arg.raw.startsWith("(IX") ? 0xdd : 0xfd;
                const disp = (arg.disp ?? 0) & 0xff;
                (0, emit_1.emitBytes)(ctx, [prefix, 0xcb, disp, base | 0x06], node.pos);
                return;
            }
            const reg = (0, utils_1.regCode)(arg.raw);
            (0, emit_1.emitBytes)(ctx, [0xcb, base | reg], node.pos);
        },
        estimate: (_ctx, [arg]) => (arg.kind === operandKind_1.OperandKind.IDX ? 4 : 2),
    },
    {
        match: () => true,
...
exports.cbBitInstr = [
    {
        match: (_ctx, args) => args.length === 2 &&
            (args[1].kind === operandKind_1.OperandKind.REG8 || args[1].kind === operandKind_1.OperandKind.REG_IND || args[1].kind === operandKind_1.OperandKind.IDX),
        encode(ctx, [bitArg, arg], node) {
            ...
            if (arg.kind === operandKind_1.OperandKind.IDX) {
                const prefix = arg.raw.startsWith("(IX") ? 0xdd : 0xfd;
                const disp = (arg.disp ?? 0) & 0xff;
                (0, emit_1.emitBytes)(ctx, [prefix, 0xcb, disp, base | (bit << 3) | 0x06], node.pos);
                return;
            }
            const reg = (0, utils_1.regCode)(arg.raw);
            (0, emit_1.emitBytes)(ctx, [0xcb, base | (bit << 3) | reg], node.pos);
        },
        estimate: (_ctx, args) => (args[1].kind === operandKind_1.OperandKind.IDX ? 4 : 2),
    },
    {
        match: () => true,
        encode(_ctx, args, node) {
...
function encodeCB(ctx, node) {
    const defs = ["BIT", "RES", "SET"].includes(node.op.toUpperCase()) ? exports.cbBitInstr : exports.cbRotInstr;
