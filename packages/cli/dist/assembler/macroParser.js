...
function parseTokens(tokens, ctx, opts) {
...
function parse(ctx, tokens) {
    ...
    function parseMacroParamsFromLine(tokens, pos) {
        const params = [];
        if (tokens.length === 0)
            return params;
        ...
        for (const seg of segments) {
            ...
            if (nameTok.kind !== "ident") {
                throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.MacroInvalidParamName, `Invalid macro parameter name: ${seg.map(t => t.text).join("")}`, { pos });
            }
            const name = nameTok.text;
            if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)) {
                throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.MacroInvalidParamName, `Invalid macro parameter name: ${name}`, { pos: nameTok.pos });
            }
            if (seg.length === 1) {
                params.push({ name });
                continue;
            ...
            if (seg[1].kind !== "colon") {
                throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.MacroInvalidParamName, `Invalid macro parameter syntax: ${seg.map(t => t.text).join("")}`, { pos: nameTok.pos });
            }
    ...
    function parseMacroInvokeArgs(tokens) {
        if (tokens.length === 0)
            return [];
        const segments = splitByComma(tokens);
        return segments.map(seg => seg.map(t => t.text).join("").trim());
    }
    function flushLine(eolPos) {
        if (line.length === 0) {
            if (eolPos) {
        ...
        if (line[0].kind === "ident" && loopOps.includes(line[0].text.toUpperCase())) {
            ...
            // 終端が見つからないのはエラー
            if (!endPos) {
                throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.SyntaxError, `Missing ${terminators[0]} for ${op}`, { pos: line[0].pos });
            }
            // ノード化
            const loopNode = {
                kind: "macroLoop",
        ...
        if (line.length >= 2 &&
            ((line[0].kind === "ident" &&
                line[1].kind === "ident" &&
                line[1].text.toUpperCase() === "MACRO") || (line[0].kind === "ident" &&
            ...
            if (!foundEndm) {
                throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.MacroEndmMissing, `Missing ENDM for macro '${macroName}'`, { pos: startPos });
            }
        }
        ...
        if (line.length >= 1 && line[0].kind === "ident") {
            ...
            const def = (0, macro_1.getDefByName)(ctx, name); // ← ★ ローカルも見える！
            if (def) {
        ...
        if (line.length >= 2 &&
            ...
            if (argTok.kind !== "string") {
                throw (0, errors_1.makeError)(errors_1.AssemblerErrorCode.SyntaxError, `INCLUDE expects a string literal at line ${argTok.pos.line}`);
            }
