...
function makeNode(op, opts) {
    return {
        kind: "macroLoop",
        op,
        bodyTokens: opts.bodyTokens ?? [],
        pos: { file: "TEST.asm", line: 1, phase: "analyze" },
        ...opts,
    };
}
function tokensAsText(nodes) {
    const o = [];
...
describe("🧩 Loop macros (REPT / WHILE / IRPC)", () => {
    ...
    test("WHILE: 無限ループ防止ガード（options.loopLimit）で例外", () => {
        const ctx = (0, context_1.createAsmContext)({ options: { loopLimit: 5 } });
        const node = makeNode("WHILE", {
            condExpr: { value: () => true, text: "true" }, // 永遠に true
            bodyTokens: [{ text: "X" }],
        });
        expect(() => (0, expandLoopCore_1.expandLoopCore)(node, ctx)).toThrow(); // LoopLimitExceeded を想定
    });
    ...
    test("REPT: 負数 count はエラー", () => {
        ...
        expect(() => (0, expandLoopCore_1.expandLoopCore)(node, ctx)).toThrow();
    });
    ...
    test("Nested REPT: 外層/内層の \\##n インデックスを正しく置換", () => {
        ...
        for (let outer = 0; outer < 2; outer++) {
            ctx.loopStack = [
                { index: outer, maxIndex: 1, locals: new Map() } // outer
            ];
            for (let inner = 0; inner < 3; inner++) {
                const frame = { index: inner, maxIndex: 2, locals: new Map() };
                ...
                // 実装ロジックを使用して置換
                (0, expandLoopCore_1.substituteLoopTokens)(toks, ctx, frame);
                out.push(...toks);
                ctx.loopStack.pop();
            }
        }
    ...
    test("REPT: \\##MAX が最外層ループ最大値を返す", () => {
        ...
        (0, expandLoopCore_1.substituteLoopTokens)(tokens, ctx, ctx.loopStack.at(-1));
