...
describe("emit.ts 基本動作テスト", () => {
  ...
  test("emitBytesでバイト列が出力され、LCとtextsが更新される", () => {
    emitBytes(ctx, [0x3E, 0x00], { line: 0, file: "test.asm", phase: "emit" });
    const text = ctx.sections.get(0)!;
  ...
  test("emitWordで16bit値が小端で出力される", () => {
    emitWord(ctx, 0x1234, { line: 0, file: "test.asm", phase: "emit" });
    const text = ctx.sections.get(0)!;
    expect(text.bytes).toEqual([0x34, 0x12]);
  });

  test("emitFixupで未解決シンボルが登録され、仮データが出力される", () => {
    const pos: SourcePos = { line: 0, file: "test.asm", phase: "emit" };
  ...
  test("emitSectionで既存セクションに戻るとLCが維持される", () => {
    ...
    // DATAセクション作成
    emitSection(ctx, "DATA");
    emitBytes(ctx, [0x11, 0x22], { line: 0, file: "test.asm", phase: "emit" });
    expect(getLC(ctx)).toBe(2);
  ...
  test("emitGapでゼロ埋めされ、LCが増える", () => {
    emitGap(ctx, 4, { line: 0, file: "test.asm", phase: "emit" });
    const text = ctx.sections.get(0)!;
  ...
  test("emitAlignで境界に揃えられる", () => {
    emitBytes(ctx, [0x01, 0x02, 0x03], { line: 0, file: "test.asm", phase: "emit" });
    emitAlign(ctx, 4, { line: 0, file: "test.asm", phase: "emit" });
    const text = ctx.sections.get(0)!;
  ...
  test("getLC / setLCでロケーションを取得・設定できる", () => {
    emitBytes(ctx, [1, 2, 3], { line: 0, file: "test.asm", phase: "emit" });
  ...
  test("複数セクション間で個別のLCが保持される", () => {
    emitBytes(ctx, [1, 2, 3], { line: 0, file: "test.asm", phase: "emit" });
    emitSection(ctx, "DATA");
    emitBytes(ctx, [0xAA], { line: 0, file: "test.asm", phase: "emit" });
