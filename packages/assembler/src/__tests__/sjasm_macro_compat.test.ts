...
`,
  ...
  it("matches sjasmplus Issue45B_arg_substitution fixture bytes", () => {
    ...
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), "mz80-sjasm-fixture-"));
    const normalizedInput = path.join(outDir, "Issue45B_arg_substitution.asm");
    const outFile = path.join(outDir, "fixture.rel");

    try {
      // The upstream macro fixture uses sjasmplus's non-standard two-character
      // literal only for ORG. Preserve the macro regression while keeping the
      // assembler's documented single-character literal contract.
      fs.writeFileSync(
        normalizedInput,
        fs.readFileSync(inputFile, "utf8").replace(/^\s*ORG 'PS'.*$/m, "ORG 0x5053"),
      );
      const ctx = assemble(createLogger("quiet"), normalizedInput, outFile, { relVersion: 2 });
      const actual = (ctx.texts ?? []).flatMap((t) => t.data);
      expect(ctx.errors).toEqual([]);
      expect(actual).toEqual(expected);
    } finally {
