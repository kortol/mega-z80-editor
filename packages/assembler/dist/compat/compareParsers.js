...
function runOnce(name, src, tag, outDir, opts, virtualFiles) {
    const logger = (0, core_1.createLogger)("quiet");
    ...
    try {
        const ctx = (0, assemble_1.assemble)(logger, tmpAsm, relPath, options);
        return {
...
function runFileOnce(name, inputFile, tag, outDir, opts) {
    ...
    try {
        const ctx = (0, assemble_1.assemble)(logger, inputFile, relPath, options);
        return {
            errors: ctx.errors.map(e => `${e.code}:${e.message}`),
            warnings: ctx.warnings.map(w => `${w.code}:${w.message}`),
            outputs: {
