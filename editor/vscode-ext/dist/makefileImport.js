...
function importProjectFromSimpleMakefile(workspaceRoot, baseConfig) {
    ...
    for (const rule of rules) {
        if (targets[sanitizeTargetName(path.basename(rule.target, path.extname(rule.target)))])
            continue;
        for (const commandLine of rule.commands) {
            const tokens = tokenizeShell(commandLine);
...
function findMakefilePath(workspaceRoot) {
...
function detectUnsupportedMakefile(workspaceRoot, content) {
...
function parseRules(content, vars) {
    const lines = content.split(/\r?\n/);
    const rules = [];
    let current;
    for (const line of lines) {
...
function expandVariables(input, vars, depth = 0) {
    if (depth > 8)
        return input;
...
function parseMz80Command(tokens) {
    const info = findMz80Invocation(tokens);
    if (!info)
        return undefined;
    const args = tokens.slice(info.argsIndex);
    if (info.subcommand === "as")
        return parseAssemblerCommand(args);
    if (info.subcommand === "link")
        return parseLinkCommand(args);
    return undefined;
}
function parseSimpleZ80asmCommand(tokens) {
    if (tokens.length === 0)
        return undefined;
...
function findMz80Invocation(tokens) {
    for (let i = 0; i < tokens.length; i++) {
        const base = path.basename(tokens[i]).toLowerCase();
        if (base !== "mz80" && base !== "mz80.cmd" && base !== "index.js")
            continue;
        let next = i + 1;
        if (tokens[next] === "--")
            next++;
...
function parseAssemblerCommand(args) {
    const options = {};
    const positional = [];
    for (let i = 0; i < args.length; i++) {
        ...
        switch (token) {
            ...
            case "--inc":
                ...
                if (args[i + 1])
                    options.includePaths.push(args[++i]);
                break;
            default:
                if (token.startsWith("-")) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        switch (token) {
            ...
            case "--org-custom":
                ...
                break;
            case "--fullpath":
                options.fullpath = (args[i + 1] && !args[i + 1].startsWith("-") ? args[++i] : "rel");
                break;
            default:
                if (token.startsWith("-")) {
...
function foldAssemblyOptions(options) {
    if (options.length === 0)
        return undefined;
...
function normalizeAsOptions(options) {
    return {
...
function detectCleanRule(rules) {
