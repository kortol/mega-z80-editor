import { SemanticTokens, SemanticTokensParams } from "vscode-languageserver/node.js";
import { TextDocument } from "vscode-languageserver-textdocument";
import { AsmTokenLike, ParsedNodeLike, createPipeline, runAnalysis } from "./asmPipeline";

type SemanticTokenTypeName = "variable" | "function" | "parameter";
type SemanticTokenModifierName = "declaration" | "readonly";
...
const tokenTypes: SemanticTokenTypeName[] = ["variable", "function", "parameter"];
const tokenModifiers: SemanticTokenModifierName[] = ["declaration", "readonly"];
...
function isReferenceCandidate(token: AsmTokenLike): boolean {
  if (token.kind !== "ident") return false;
  if (token.text === "$") return false;
  if (token.text.startsWith("\\")) return false;
  const upper = normalize(token.text);
  if (registerNames.has(upper) || conditionNames.has(upper)) return false;
  return /^[.@]?[A-Za-z_@%][A-Za-z0-9_.$@%]*$/u.test(token.text);
}

function keyOf(token: AsmTokenLike): string {
  return `${token.pos.line}:${token.pos.column ?? 0}:${token.text.length}`;
}

function groupTokensByLine(tokens: AsmTokenLike[]): Map<number, AsmTokenLike[]> {
  const byLine = new Map<number, AsmTokenLike[]>();
  for (const token of tokens) {
    if (token.kind === "eol") continue;
...
function buildMacroMaps(nodes: ParsedNodeLike[]): {
  defs: Set<string>;
  invokesByLine: Map<number, Set<string>>;
} {
  const defs = new Set<string>();
  const invokesByLine = new Map<number, Set<string>>();
  ...
  for (const node of nodes) {
    ...
    if (node.kind === "macroInvoke") {
      ...
      const bucket = invokesByLine.get(line) ?? new Set<string>();
...
function addToken(
  ...
  const entry: EncodedToken = { line, start, length, type, modifiers, priority };
...
function collectExplicitTokens(
  lineTokens: AsmTokenLike[],
  macroInvokesOnLine: Set<string> | undefined,
  store: Map<string, EncodedToken>
): void {
  ...
  if (!head || head.kind !== "ident") return;
...
function encodeTokens(store: Map<string, EncodedToken>): SemanticTokens {
  const ordered = [...store.values()].sort((a, b) =>
    a.line - b.line || a.start - b.start || a.length - b.length
  );
  const data: number[] = [];
  let prevLine = 0;
  let prevStart = 0;
  ...
  for (const token of ordered) {
    ...
    const modifiers = token.modifiers.reduce(
      (bits, modifier) => bits | (tokenModifierBit.get(modifier) ?? 0),
      0
    );
...
export function collectSemanticTokens(
  ...
  const store = new Map<string, EncodedToken>();
  ...
  for (const token of tokens) {
    if (!isReferenceCandidate(token)) continue;
    const key = keyOf(token);
