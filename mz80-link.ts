...
interface RelText { addr: number; bytes: number[]; }
interface RelRef { addr: number; sym: string; }
...
// ========== .rel パーサ ==========
function parseRelFile(filename: string): RelModule {
  ...
  for (const line of lines) {
    ...
    switch (rec) {
      ...
      case "T": {
        ...
        const bytes = rest.slice(1).map((x: string) => parseInt(x, 16));
